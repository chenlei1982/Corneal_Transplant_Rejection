...
def selection_feature(data,method):
    ...
    if method =='AdaBoost':
        ...
        Ada_clf.fit(x,y)
        feature_import=Ada_clf.feature_importances_

    if method=='ExtraTrees':
        extra_clf=ExtraTreesClassifier()
        extra_clf.fit(x,y)
        feature_import=extra_clf.feature_importances_
             
    if method=='Ridge':
        ridge=RidgeCV()
        ridge.fit(x,y)
        feature_import=np.abs(ridge.coef_)
...
def main(args):
    ...
    data=df.fillna(0)
    methods=['RF','XGBoost','CATboost','AdaBoost','ExtraTrees','Ridge']
